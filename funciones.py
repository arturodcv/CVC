...
def collect_data(image_selected, exc_eeg, inh_eeg, peaks_exc, freqs_exc,peaks_tot,freqs_tot, idx, seed):
    dictionary = {'image_name': image_selected, 'exc_activity': np.sum(exc_eeg), 'inh_activity': np.sum(inh_eeg), 
                  ...
                  'node_exc': peaks_exc[idx][0], 'gamma_power_exc': np.around(sum(freqs_exc[broadband_initial:broadband_end_1]),2) ,
                  'node_tot': peaks_tot[idx][0], 'gamma_power_tot': np.around(sum(freqs_tot[broadband_initial:broadband_end_1]),2) ,
                  'seed': seed, 
